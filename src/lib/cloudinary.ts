...
export async function generateCloudinaryZip(
  ...
  const { cloudName, apiKey, apiSecret } = ensureCloudinaryConfig()
  const endpoint = new URL(`https://api.cloudinary.com/v1_1/${cloudName}/image/generate_archive`)
  ...
  if (!res.ok) {
    ...
    throw new Error(`Cloudinary generate_archive failed: ${res.status} ${res.statusText} ${errText}`)
  }

  const data = (await res.json()) as { url?: string; archive_url?: string; expires_at?: string }
...
export function normalizeCloudinaryResource(
  ...
  const ctx = (resource as unknown as { context?: { custom?: Record<string, unknown> } }).context?.custom || {}
  const meta = (resource as unknown as { metadata?: Record<string, unknown> }).metadata || {}
  ...
  if (typeof (meta as any).tags === 'string') tagStrings.push((meta as any).tags)
  ...
  const isTrue = (v: unknown) => v === true || v === 1 || v === '1' || (typeof v === 'string' && v.toLowerCase() === 'true')
  for (const k of featureKeys) {
    if (isTrue((ctx as any)[k]) || isTrue((meta as any)[k])) {
      if (!tags.map((t) => t.toLowerCase()).includes('featured')) tags.push('featured')
      break
    }
  }
  ...
  const category = (catOverrideRaw && typeof catOverrideRaw === 'string'
    ? catOverrideRaw
...
export function buildTransformation(opts: TransformOptions): string {
  const parts: string[] = []
  const mapFit: Record<string, string> = { fill: 'c_fill', pad: 'c_pad', fit: 'c_fit' }
  ...
  if (typeof opts.saturation === 'number') parts.push(`e_saturation:${opts.saturation}`)
  if (typeof opts.hue === 'number') parts.push(`e_hue:${opts.hue}`)
  if (typeof opts.brightness === 'number') parts.push(`e_brightness:${opts.brightness}`)
  if (typeof opts.contrast === 'number') parts.push(`e_contrast:${opts.contrast}`)
