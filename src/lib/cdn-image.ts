...
export function gridThumbUrl(url: string, device: 'desktop' | 'mobile' = 'desktop'): string {
  ...
  return withCloudinaryTransform(url, `${base},${size}`)
}

export function previewImageUrl(url: string, device: 'desktop' | 'mobile' = 'desktop'): string {
