...
export default function GalleryClient({ baseWallpapers, fixedDevice, title, initialDevice, showControls = true }: GalleryClientProps) {
  ...
  const preloadImage = (url: string, device: 'desktop' | 'mobile' = 'desktop') => {
          ...
          {filteredWallpapers.map((wallpaper) => (
            <StaggerItem key={wallpaper.id}>
              <div 
                ...
                onMouseEnter={() => preloadImage(wallpaper.thumbnailPath, (wallpaper.deviceType ?? 'desktop'))}
              >
                <div
                  className={cn(
                ...
                >
                  <div
                    className={cn(
                      ...
                      (wallpaper.deviceType ?? 'desktop') === 'desktop' ? 'aspect-[16/9]' : 'aspect-[4/5]'
                    )}
                  >
                    <Image
                      src={gridThumbUrl(wallpaper.thumbnailPath, (wallpaper.deviceType ?? 'desktop'))}
                      alt={wallpaper.title}
                      fill
                      priority={false}
                      unoptimized
                  ...
                  <div className="p-4 mt-auto">
                    ...
                    <div className="grid grid-cols-[1fr_auto] gap-x-3 gap-y-1 text-[12px] leading-tight text-foreground/85 font-mono uppercase tracking-wide">
                      ...
                      <span className="font-semibold">{(wallpaper.deviceType ?? 'desktop').toUpperCase()}</span>
                      <span className="opacity-80">{wallpaper.downloads} DL</span>
                      {wallpaper.deviceType ? (
                        <span className="opacity-80">{wallpaper.deviceType === 'mobile' ? 'PORTRAIT' : 'LANDSCAPE'}</span>
                      ) : (
